...
// systeme allopass - n'oublier pas de changer votre code script et autre
...
defined("INC") or die("403 restricted access");
		...
		if ( isset ( $_GET['action'] ) )
			{
				...
				if ( $_GET['action'] == "send" )
				{
					 ...
					 $code3 = "2258711";
					  ...
					  setCookie( "CODE_OK", "1", 0, "/", ".cmd-web.info", false );
	...
	while ($sql_corp = mysql_fetch_array($reponse_corp) )
		...
		$facture_id_prix = $sql_corp['id_prix'];
				$reponse_prix = mysql_query("SELECT * FROM prix_service WHERE id='$facture_id_prix' "); // Requête SQL
						while ($sql_prix = mysql_fetch_array($reponse_prix) )
						{
						//prix du vps
						$prix_vps +=  $sql_prix['prix'];
				
						$init_price_verification += 1;
						}
						if ( $init_price_verification != 1 )
						{
						$error_msg .= "<br /><strong>Erreur : </strong>Erreur l'ors de la récuperation du prix.<br />";
						$erreur += 	1;
						}
		}			
  ...
  <a href="javascript:;" onclick="javascript:window.open('http://payment.allopass.com/acte/scripts/popup/access.apu?ids=170550&amp;idd=469813&amp;lang=fr&amp;country=co','phone','toolbar=0,location=0,directories=0,status=0,scrollbars=0,resizable=0,copyhistory=0,menuBar=0,width=300,height=340');"><img border="0" src="http://payment.allopass.com/imgweb/common/flag_co.gif" width="35" height="29" alt="CO" title="CO" /></a>
	...
	echo '<table border="0" cellpadding="0" cellspacing="0" width="149" height="80">
    <tr>
        <td width="149" height="80">
            ...
            <input type="hidden" name="ids" value=" 170550">
                <input type="hidden" name="idd" value="780557">
                <input type="hidden" name="lang" value="fr">
                <input type="image" src="https://allopass.com/imgweb/script/fr/cb_subscribe_os.gif" alt="Saisir votre ticket daccès " onClick="window.open(\'\',\'DisplaySub\',\'toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable=1,width=600,height=570\');" border = 0>
            </form>
        </td>
    </tr>
</table>
